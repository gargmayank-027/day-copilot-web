      ...
      <div style={{ width:"100%", maxWidth:430, padding:"60px 24px 40px", display:"flex", flexDirection:"column", justifyContent:"center" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <button onClick={handleSubmit} disabled={loading} style={{
            ...
            background:T.gradViolet, color:"#fff",
    ...
    <div style={{
      ...
      >
        <div style={{
          ...
          border: task.done ? "none" : `1.5px solid ${T.text3}`,
          background: task.done ? T.gradViolet : "transparent",
...
function MoodSelector({ mood, setMood }) {
  const moods = [
    { key:"high",    Icon:Smile, label:"Energized", color:T.green, soft:T.greenSoft },
    { key:"neutral", Icon:Meh,   label:"Steady",    color:T.amber, soft:T.amberSoft },
    { key:"low",     Icon:Frown, label:"Drained",   color:T.red,   soft:T.redSoft   },
  ];
  return (
      ...
      {moods.map(({ key, Icon, label, color, soft }) => {
        ...
        return (
          <button key={key} onClick={() => setMood(key)} style={{
            flex:1, border: a ? `1.5px solid ${color}` : `1px solid ${T.border}`,
            borderRadius:16, padding:"14px 6px",
            background: a ? soft : T.bg2, cursor:"pointer",
    ...
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
          ...
          <span style={{ fontSize:13, fontWeight:700, color:T.text1 }}>Momentum</span>
        </div>
...
function AddTaskSheet({ onClose, onAdd }) {
  ...
  const iStyle = {
    ...
    fontSize:14, color:T.text1, outline:"none", transition:"border 0.2s",
  };
...
/* ─── HOME VIEW ───────────────────────────────────────────────────────────── */
function HomeView({ tasks, onToggle, mood, setMood, userProfile }) {
      ...
      {sections.map(({ key, label, Icon, color, soft }) => (
        <Card key={key} style={{ padding:"16px 18px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:4 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <p style={{ fontSize:11, color:T.text2 }}>
                {tasks[key]?.filter(t=>t.done).length||0}/{tasks[key]?.length||0} done
                · {tasks[key]?.reduce((s,t)=>s+t.duration,0)||0}m total
              </p>
            </div>
          ...
          {(!tasks[key] || tasks[key].length === 0) && (
            <p style={{ fontSize:13, color:T.text3, padding:"10px 0 2px", textAlign:"center" }}>No tasks yet — tap + to add one</p>
          )}
        </Card>
      ))}
    </div>
...
function InsightsView({ tasks, mood }) {
  ...
  const byTag     = {};
  all.forEach(t => { byTag[t.tag] = (byTag[t.tag]||0)+1; });
    ...
    <div style={{ animation:"fadeUp 0.3s ease" }}>
      ...
      <Card>
        ...
        <div style={{ display:"flex", gap:10 }}>
          {[
            { label:"done",      value:`${focusDone}m`, color:T.green,  soft:T.greenSoft },
            { label:"remaining", value:`${focusLeft}m`, color:T.text2,  soft:T.bg2       },
...
function ProfileView({ userProfile, userEmail, onSignOut }) {
  ...
  return (
    <div style={{ animation:"fadeUp 0.3s ease" }}>
        ...
        {userProfile?.goals?.length > 0 && (
          <div style={{ display:"flex", flexWrap:"wrap", gap:6, justifyContent:"center", marginTop:10 }}>
            {userProfile.goals.map(g => (
              <span key={g} style={{ fontSize:11, fontWeight:600, color:T.violetMid, background:T.violetSoft, padding:"3px 10px", borderRadius:99, textTransform:"capitalize" }}>{g}</span>
            ))}
          </div>
        )}
      </div>
      ...
      {/* Schedule from onboarding */}
      {userProfile?.work_start && (
        <Card>
          <SLabel icon={Clock} label="Your Schedule"/>
          <div style={{ display:"flex", gap:10, marginBottom:10 }}>
            {[
              { label:"Work start", value: userProfile.work_start },
              { label:"Work end",   value: userProfile.work_end   },
            ].map(({ label, value }) => (
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            ...
            <div>
              <p style={{ fontSize:14, fontWeight:700, color:T.text1 }}>
                {userProfile.energy_type === "early" ? "Early bird" : userProfile.energy_type === "night" ? "Night owl" : "Midday warrior"}
              </p>
              <p style={{ fontSize:12, color:T.text2, marginTop:1 }}>Your peak focus time is scheduled accordingly</p>
            </div>
          </div>
        </Card>
      )}
      ...
      <Card>
        ...
        {[
          { label:"Focus session", val:focusDur, set:setFocusDur, min:15, max:120, step:5, color:T.violetMid },
          { label:"Break length",  val:breakDur, set:setBreakDur, min:5,  max:30,  step:5, color:T.green     },
      ...
      <Card>
        {[
          {
            label:"Notifications", sub:"Task reminders & nudges",
            right: <div onClick={() => setNotifs(!notifs)} style={{ width:46, height:26, borderRadius:99, cursor:"pointer", background: notifs ? T.violetMid : T.bg3, display:"flex", alignItems:"center", padding:"0 4px", justifyContent: notifs ? "flex-end" : "flex-start", transition:"all 0.25s cubic-bezier(0.34,1.56,0.64,1)", boxShadow: notifs ? `0 0 10px ${T.violetGlow}` : "none" }}><div style={{ width:18, height:18, borderRadius:"50%", background:"#fff" }}/></div>,
          },
          {
            label:"Theme", sub:null,
            right: <span style={{ fontSize:12, fontWeight:700, color:T.violetMid, background:T.violetSoft, padding:"4px 12px", borderRadius:99 }}>Midnight</span>,
          },
          ...
          <div key={label} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 0", borderBottom: i < arr.length-1 ? `1px solid ${T.border}` : "none" }}>
            <div>
              <p style={{ fontSize:14, fontWeight:600, color:T.text1 }}>{label}</p>
              {sub && <p style={{ fontSize:12, color:T.text2, marginTop:1 }}>{sub}</p>}
            </div>
            {right}
          </div>
        ))}
...
export default function App() {
  ...
  /* ── 2. Check onboarding + load profile when session exists ───────────── */
  useEffect(() => {
  ...
  const handleAdd = async (section, taskData) => {
    ...
    const { data, error } = await supabase.from("tasks").insert({
      ...taskData, section, user_id: session.user.id,
    }).select().single();
    if (!error && data) {
  ...
  const handleOnboardingComplete = async () => {
    // Re-fetch profile after onboarding saves it
  ...
  /* ── Main app ─────────────────────────────────────────────────────────── */
  const screens = {
    home:     <HomeView     tasks={tasks} onToggle={handleToggle} mood={mood} setMood={setMood} userProfile={userProfile}/>,
